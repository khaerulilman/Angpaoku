  ...
  <div class="min-h-screen bg-surface text-on-background">
    <nav class="fixed inset-x-0 top-0 z-50 px-6 py-4">
      <div class="mx-auto flex max-w-7xl items-center justify-between">
        ...
        <p class="rounded-full bg-surface-container-lowest px-4 py-2 text-xs font-semibold text-on-surface-variant">
    ...
    <main class="mx-auto max-w-7xl px-6 pb-24 pt-24">
      <header class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-primary/10 via-surface-container-low to-tertiary/10 p-8 md:p-12">
        <div class="absolute -right-16 -top-16 h-44 w-44 rounded-full bg-primary/10 blur-2xl"></div>
        <div class="absolute -bottom-16 -left-10 h-44 w-44 rounded-full bg-tertiary/10 blur-2xl"></div>

        <div class="relative flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          <div class="space-y-3">
            <p class="text-xs font-bold uppercase tracking-[0.22em] text-on-surface-variant">Creator Store</p>
            <h1 class="text-3xl font-extrabold tracking-tight text-on-surface md:text-5xl">
              {{ profileDisplayName }}
            </h1>
          ...
          <div class="grid grid-cols-3 gap-3">
            ...
            >
              <p class="text-xl font-extrabold text-on-surface md:text-2xl">{{ stat.value }}</p>
              <p class="text-[10px] font-semibold uppercase tracking-widest text-on-surface-variant md:text-xs">
                {{ stat.label }}
              </p>
            </div>
          </div>
      ...
      <section class="mt-12">
        <div class="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h2 class="text-2xl font-bold tracking-tight text-on-surface md:text-3xl">Public Products</h2>
            <p class="text-sm text-on-surface-variant">List product dengan visibility public (live).</p>
          </div>
        ...
        >
          ...
          >
            ...
            <div class="space-y-3 p-5">
              <div class="h-4 w-2/3 animate-pulse rounded bg-surface-container"></div>
              <div class="h-3 w-full animate-pulse rounded bg-surface-container"></div>
              <div class="h-3 w-4/5 animate-pulse rounded bg-surface-container"></div>
            </div>
          </div>
        </div>
        ...
        >
          <p class="text-lg font-semibold text-on-surface">Belum ada produk public.</p>
          <p class="mt-2 text-sm text-on-surface-variant">
            Produk akan muncul di halaman ini ketika visibility diset ke <strong>live</strong>.
          </p>
        </div>
        ...
        <div v-else class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          ...
          >
            <div class="relative h-56 overflow-hidden bg-surface-container">
              ...
              <div class="absolute left-3 top-3 rounded-full bg-surface-container-lowest/90 px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-primary">
            ...
            <div class="space-y-4 p-5">
              ...
              <div class="flex items-end justify-between gap-4">
                <div>
                  ...
                  <p class="text-xl font-extrabold text-on-surface">{{ displayPrice(product) }}</p>
                </div>
...
import { storePreviewApi, type ProductRecord } from '@/api'
...
const filteredProducts = computed(() => {
  ...
  return products.value.filter((product) => (product.category?.name ?? '') === selectedFilter.value)
})

const storeStats = computed(() => {
  const total = products.value.length
  const free = products.value.filter((product) => product.pricing_type === 'free').length
...
function getFinalPrice(product: ProductRecord): number {
  ...
  const discounted = Math.round((product.price * (100 - product.discount_percentage)) / 100)
...
async function loadStorePreview(): Promise<void> {
  ...
  try {
    ...
    profileName.value = ''
    products.value = []
    errorMessage.value = error instanceof Error ? error.message : 'Gagal memuat store preview.'
  } finally {
